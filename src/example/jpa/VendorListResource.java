...
public class VendorListResource {
	...
	@POST
	public Response create(@FormParam("name") String name, @FormParam("city") String city, @FormParam("latitude") String latitude, 
		...
		try {
			...
			em.persist(vendor);
			utx.commit();
			return Response.ok(vendor.toString()).build();
		} catch (Exception e) {
			e.printStackTrace();			
 ...
 // UPDATE function
	...
	public Response update(@FormParam("id") long id, @FormParam("name") String name, @FormParam("city") String city, @FormParam("latitude") String latitude, 
		...
		try {
			...
			Vendor vendor = em.find(Vendor.class, id);
			if (vendor != null) {
				vendor.setName(name);// TODO check if null
				vendor.setCity(city);
				vendor.setLatitude(latitude);
				vendor.setLongitude(longitude);
				vendor.setPhone(phone);
	...
	public Response get(@QueryParam("id") long id) {
		if (id == 0) {
			List<Vendor> list = em.createQuery("SELECT t FROM Vendor t", Vendor.class).getResultList();
			if (list.size() == 0) {
				createSampleData();
				list = em.createQuery("SELECT t FROM Vendor t", Vendor.class).getResultList();
			}
	...
	private void createSampleData() {
		create("Vendor 1", "City 1", "Lat 1", "Long 1", "Phone 1");
		create("Vendor 2", "City 2", "Lat 2", "Long 2", "Phone 2");
		create("Vendor 3", "City 3", "Lat 3", "Long 3", "Phone 3");
	}
